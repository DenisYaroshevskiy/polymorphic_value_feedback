...
// https://github.com/jbcoe/polymorphic_value/blob/15e73d93d8dcc819e0e5047ad12024bdd8d7ff99/test_polymorphic_value.cpp#L1
//
// I only changed a few things to apply it to my implementation.
// Feel free to reach out to me if you have a license issue.
...
struct DerivedType : BaseType
{
  ...
  friend bool operator==(const DerivedType& x, const DerivedType& y) {
...
TEST_CASE("polymorphic_value assignment","[polymorphic_value.assignment]")
{
  ...
  GIVEN("A pointer-constructed polymorphic_value assigned to itself")
  {
    ...
    auto origin = std::make_unique<DerivedType>(v1);
    const DerivedType test_against = *origin;

    polymorphic_value<BaseType> cptr1(origin.release());
...
#pragma clang diagnostic pop
    ...
    // CHECK(DerivedType::object_count == 3);

    THEN("The assigned-from object is unchanged")
    {
      REQUIRE(static_cast<DerivedType&>(*cptr1) == test_against);
    }
  }
}
